...
const main = async () => {
    ...
    data["contractAddress"] = "kovan";
    data["msg"] = web3.utils.sha3(accounts[0].address);
    ...
    for(var i = 0 ; i < len ;i ++){
        ...
        eachSign["address"] = accounts[i].address
        let publicKey = util.bufferToHex(util.privateToPublic(privateKeyList[i]));
        eachSign["publicKey"] = publicKey;

        var signature = (await web3.eth.sign(data["msg"],accounts[i].address))
        eachSign["signature"] = signature
        signs.push(eachSign);
    ...
    fs.writeFileSync(file, JSON.stringify(data, null, 4), { encoding: 'utf8' }, err => {})
